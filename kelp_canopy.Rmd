---
title: "Kelp Canopy Cover Summary Using Fishing Block Shapefiles"
...
```{r Load Landsast data into fishing block shapefiles, include = FALSE}
# ## THIS HAS ALL BEEN COMMENTED BECAUSE IT IS JUST THE WORK THAT LED TO THE CSVS I'M USING FOR THE FIGS
# #Load fishing block shapefiles
# fish_blocks <- st_read('caltrawl_GCS.shp')
# 
...
# fb_nc_data <- nc_open('LandsatKelpBiomass_2022_Q1_withmetadata.nc') # through Q1 2022
# 
# #Load necessary variables from NetCDF File
# fb_lon      <- ncvar_get(fb_nc_data, 'longitude')
# fb_lat      <- ncvar_get(fb_nc_data, 'latitude')
# fb_years    <- ncvar_get(fb_nc_data, 'year')
# fb_quarters <- ncvar_get(fb_nc_data, 'quarter')
# fb_biomass  <- ncvar_get(fb_nc_data, 'biomass')
# fb_area     <- ncvar_get(fb_nc_data, 'area')
# 
# #Make Coordinate Dataframe
# fb_coords <- data.frame(fb_lon,fb_lat)
# 
# #Create output matrices
# fb_LN = dim(fb_quarters)
# fb_output_biomass <- matrix(ncol = fb_LN, nrow = 547)
# fb_output_area    <- matrix(ncol = fb_LN, nrow = 547)
# 
# # The below loop takes 10 minutes to run.
# for(i in 1:547){
# 
#   #Pull shapefile coordinates from each fishing block
#   #I had to make hard code an addition here for four blocks as I (Tom Bell) am not as fluent in geospatial analysis in R as I am in Matlab
#   #The issue is that the shaprefile geometry is divided into multiple parts when there are NaNs in the lat/lon columns
#   if (i == 289 | i == 306 | i == 433 | i == 498) {
#     fb_coords <- as.data.frame(fish_blocks[i,]$geometry[[1]][[2]][[1]])
#   } else {
#     fb_coords <- as.data.frame(fish_blocks[i,]$geometry[[1]][[1]][[1]])
#   }
# 
#   #Find the kelp coordinates that exist inside each fishing block polygon
#   kelp_poly <- point.in.polygon(fb_lon, fb_lat, fb_coords$V1, fb_coords$V2, mode.checked = FALSE)
#   TF <- which(kelp_poly %in% 1)
# 
#   #Extract biomass and area data for each polygon
#   bio_block <- fb_biomass[TF,]
#   area_block <- fb_area[TF,]
# 
#   #Sum biomass and area
#   if (is.null(dim(area_block)) == TRUE) {
#     fb_bio_sum <- bio_block
#     fb_area_sum <- area_block
#   } else {
#     fb_bio_sum  <- colSums(bio_block,  na.rm = TRUE, dims = 1)
#     fb_area_sum <- colSums(area_block, na.rm = TRUE, dims = 1)
#   }
# 
#   #Remove Columns w/ >25% No Data (Clouds)
#   #Find all NA values (missing data) in the kelp dataset
#   fb_bio_NA  = is.na(bio_block)
#   fb_area_NA = is.na(area_block)
# 
...
#   if (is.null(dim(area_block)) == TRUE) {
#     fb_bio_NA_percent  <- fb_bio_NA
#     fb_area_NA_percent <- fb_area_NA
#   } else {
#     fb_bio_NA_percent  <- colSums(fb_bio_NA,  na.rm = TRUE)/L[1]
#     fb_area_NA_percent <- colSums(fb_area_NA, na.rm = TRUE)/L[1]
#   }
# 
#   #Find seasons with greater than 25% missing data
#   fb_NA_GT  <- which(fb_bio_NA_percent > 0.25)
#   fb_NA_GTA <- which(fb_area_NA_percent > 0.25)
# 
#   #Set seasons with greater than 25% missing data to -999
#   fb_bio_sum[fb_NA_GT]   = -999
#   fb_area_sum[fb_NA_GTA] = -999
# 
#   #Add summed kelp canopy data to matrix
#   fb_output_biomass[i,] <- fb_bio_sum
#   fb_output_area[i,]    <- fb_area_sum
# 
# }
# 
# #Transpose Quarter and Year data
# fb_quarterst <- t(fb_quarters)
# fb_yearst <- t(fb_years)
# 
# #Add Quarter and Year data to output matrices
# fb_output2_biomass <- rbind(fb_quarterst, fb_output_biomass)
# fb_output3_biomass <- rbind(fb_yearst,    fb_output2_biomass)
# fb_output2_area    <- rbind(fb_quarterst, fb_output_area)
# fb_output3_area    <- rbind(fb_yearst,    fb_output2_area)
# 
# #Add fishing block Number to output matrices
# blocksNA <- fish_blocks$BLOCK10_ID
# blocks <- c(rep(NA,2),blocksNA)
# fb_output4_biomass <- cbind(blocks, fb_output3_biomass)
# fb_output4_area <- cbind(blocks, fb_output3_area)
# 
# #Write CSV of kelp biomass time series for each fishing block
# write.csv(fb_output4_biomass, 'kelp_biomass_output_fishing_block.csv', row.names = FALSE)
# write.csv(fb_output4_area, 'kelp_area_output_fishing_block.csv', row.names = FALSE)
