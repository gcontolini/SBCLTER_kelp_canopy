---
title: "Kelp Canopy Cover Summary Using Three Region Shapefiles"
...
## Plotting
Figures were created using the above dataset in conjunction with shapefiles of state waters divided into three regions as described above. 
...
```{r kelp canopy area with max in black by region, fig.height = 3, fig.width = 10}
# create dataframe with only annual max canopy area values
canopy_area_tall_max <- canopy_area_tall %>% 
  dplyr::group_by(region, year) %>%
  dplyr::summarise(max.annual = max(km2, na.rm = T)) # but need to keep the quarter col. oh I can probably just merge.
canopy_area_tall_max <- canopy_area_tall_max %>% #remove 2022 max because there's only one quarter
                        filter(!year == 2022)
canopy_area_joined <- left_join(canopy_area_tall, canopy_area_tall_max) # but just want the row with the maximum
canopy_area_joined <- canopy_area_joined[which(canopy_area_joined$km2 == canopy_area_joined$max.annual),]


# North Coast
ggplot(canopy_area_tall[which(canopy_area_tall$region == 'North'),], aes(x = year.qtr, y = km2)) +
  geom_rect(xmin = 2014.2, xmax = 2022.5, ymin = -Inf, ymax = Inf, fill = 'pink') +
  geom_line(color = 'grey50') +
  geom_point(data = canopy_area_joined[which(canopy_area_joined$region == 'North'),], aes(x = year.qtr, y = max.annual)) +
  geom_line(data = canopy_area_joined[which(canopy_area_joined$region == 'North'),], aes(x = year.qtr, y = max.annual)) +
  scale_x_continuous(breaks = 1984:2022, expand = c(expand.a.little,0)) +
  scale_y_continuous(expand = c(expand.a.little,0)) +
  labs(x = '',  y =  bquote('Canopy area'~(km^2))) +
  ggtitle('North Coast') +
...
ggplot(canopy_area_tall[which(canopy_area_tall$region == 'Central'),], aes(x = year.qtr, y = km2)) +
  geom_line(color = 'grey50') +
  geom_point(data = canopy_area_joined[which(canopy_area_joined$region == 'Central'),], aes(x = year.qtr, y = max.annual)) +
  geom_line(data = canopy_area_joined[which(canopy_area_joined$region == 'Central'),], aes(x = year.qtr, y = max.annual)) +
  scale_x_continuous(breaks = 1984:2022, expand = c(expand.a.little,0)) +
  scale_y_continuous(expand = c(expand.a.little,0)) +
  labs(x = '',  y =  bquote('Canopy area'~(km^2))) +
...
ggplot(canopy_area_tall[which(canopy_area_tall$region == 'South'),], aes(x = year.qtr, y = km2)) +
  geom_line(color = 'grey50') +
  geom_point(data = canopy_area_joined[which(canopy_area_joined$region == 'South'),], aes(x = year.qtr, y = max.annual)) +
  geom_line(data = canopy_area_joined[which(canopy_area_joined$region == 'South'),], aes(x = year.qtr, y = max.annual)) +
  scale_x_continuous(breaks = 1984:2022, expand = c(expand.a.little,0)) +
  scale_y_continuous(expand = c(expand.a.little,0)) +
  labs(x = '',  y =  bquote('Canopy area'~(km^2))) +
  ggtitle('South Coast') +
...
```
Figure 2. Quarterly canopy area (grey) and maximum annual canopy area (black) in three regions as estimated from Landsat satellites (Santa Barbara Coastal LTER et al. 2022). Canopy area does not distinguish between bull kelp and giant kelp.  Missing values indicate no data is available. Data is updated through the first quarter of 2022.
...
# Literature Cited
Santa Barbara Coastal LTER, T. Bell, K. Cavanaugh, and D. Siegel. 2022. SBC LTER: Time series of quarterly NetCDF files of kelp biomass in the canopy from Landsat 5, 7 and 8, since 1984 (ongoing) ver 14. Environmental Data Initiative. https://doi.org/10.6073/pasta/89b63c4b49b80fb839613e9d389d9902 (Accessed 2022-05-02).