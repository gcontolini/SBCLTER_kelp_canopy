---
title: "Quarterly Kelp Canopy Cover Summary Using CDFW County Shapefiles for Remote Sensing Report"
...
# Objective
Summarize California quarterly kelp abundance trends from Landsat satellite imagery (SBC LTER et al. 2021) by county using shapefiles of state waters delineated by coastal counties for use in the Office of National Marine Sanctuaries kelp remote sensing report.

# Methods
## Satellite imagery
Kelp canopy area data are from the SBC LTER Time series of quarterly NetCDF files of kelp biomass in the canopy from Landsat 5, 7 and 8, since 1984 (ongoing) updated through all four quarters of 2020 (accessed 24 Nov 2021; Santa Barbara Coastal LTER et al. 2021; https://doi.org/10.6073/pasta/89b63c4b49b80fb839613e9d389d9902). The data represents a time series of canopy area of giant kelp, *Macrocystis pyrifera*, and bull kelp, *Nereocystis luetkeana* derived from Landsat 5 Thematic Mapper (TM), Landsat 7 Enhanced Thematic Mapper Plus (ETM+), and Landsat 8 Operational Land Imager (OLI) satellite imagery, along with relevant metadata. The kelp canopy is composed of the portions of fronds and stipes floating on the surface of the water. Canopy area (m) data are given for individual 30 x 30 meter pixels for all coastal areas of California, including the Northern and Southern Channel Islands.
...
## Plotting
Figures were created using the above dataset in conjunction with CDFW county shapefiles, which include state waters within counties. 

```{r Load county shapefiles and match with Landsast data, include = FALSE}
# The loop below does not run in RStudio because it requires the NetCDF file with all the geospatial data, and the RStudio Cloud server does not support files that large. What I am doing is downloading the latest NetCDF file from the EDI repository () and then running this code on my local machine. The output is saved as a .csv and that is what is used here. I have to do it this way in order to use GitHub because GitHub integration with RStudio is not supported on state computers. 
...
# 
# # This for() loop takes 5 mins. You could run it once and then save the output, but then it wouldn't be linked to the SBCLTER data repository anymore. 
# for(i in 1:22){
# 
...
ggplot(canopy_area_tall[which(canopy_area_tall$county == 'Del Norte'),], aes(x = year.qtr, y = km2)) +
   ...
   scale_y_continuous(breaks = seq(0, 0.015, 0.0025), expand = c(0.02,0)) +
...
ggplot(canopy_area_tall[which(canopy_area_tall$county == 'Sonoma'),], aes(x = year.qtr, y = km2)) +
   ...
   geom_line() +
   scale_x_continuous(breaks = 1984:2021, expand = c(0,0)) +
   scale_y_continuous(limits = c(0,2), breaks = seq(0, 2, 0.25), expand = c(0.02,0)) +
   labs(x = '', y =  bquote('Canopy area'~(km^2))) +
...
ggplot(canopy_area_tall[which(canopy_area_tall$county == 'Marin'),], aes(x = year.qtr, y = km2)) +
   ...
   geom_line() +
   scale_x_continuous(breaks = 1984:2021, expand = c(0,0)) +
   scale_y_continuous(limits = c(0, 0.0125), breaks = seq(0, 0.0125, 0.0025), expand = c(0.02,0)) +
   labs(x = '', y =  bquote('Canopy area'~(km^2))) +
   ggtitle('Marin County Quarterly') +
   my.theme
```

Figure 1. Quarterly canopy area in state waters in coastal counties as estimated from Landsat satellites (Santa Barbara Coastal LTER et al. 2021). Canopy area does not distinguish between bull kelp and giant kelp. Data is updated through the last quarter of 2020. Missing points indicate missing data.

```{r Annual max canopy area in each county, fig.height = 2, include = FALSE}
